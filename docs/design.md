...
| wasm 扩展 | **wasm3**（third_party 内置源码，单文件解释器） | 消息处理扩展沙箱 + wasm 认证提供者，支持动态加载/热更新；无 fuel，资源限制走 runtime 内存上限 + 调用线程隔离 |
...
- **A 段失败兜底（kLlmFailed）**：LLM RPC 失败/超时由执行层回注引擎（不再只等演进超时）。A 段(quick)失败 → 立即触发本节约占位路径（每语句一次）：有上一次占位缓存直接复用（时延≈0），无缓存则发 `quick_placeholder` 生成；P 再失败 → 摘除永不就绪的占位 clip（防堵死队首 B/C），有缓存复用、无缓存则向端侧下发 `status:degraded` 状态帧（每语句一次，不死寂）。旧语句代际（打断后）的失败消息直接丢弃。
...
stateDiagram-v2
    ...
    Offline --> Online: 3分钟内重连<br/>gen+1, 恢复上下文继续话题<br/>(AEC标定随连接失效，重连重新水印标定)
    Offline --> GCD: 超3分钟未重连
...
**方案**：会话开始录音时，网关在下行的首个音频流前注入**固定波形的多脉冲水印**（4 声 chirp、固定间隔 320ms，总时长 ~0.98s，幅度 0.9FS 留 ~10% 余量防削波），端侧正常播放；随后在录回来的 PCM 中识别该水印（多脉冲一致性投票：栅格命中 ≥count-2 且最深命中槽 ≥count-2），推算环路延迟与漂移。
...
flowchart TD
    ...
    ncc --> peaks{"多脉冲一致性投票:<br/>候选首脉冲栅格 p+k*GAP 命中≥count-2 槽<br/>且最深命中槽≥count-2(尾槽或次尾槽)"}
    peaks -->|"是"| calc["环路延迟 D = p1 - 水印播放时刻<br/>(按已下发采样计数推算)<br/>时钟漂移 skew = 相邻命中槽间隔中位数/GAP - 1"]
    calc --> msg["kWmDetected → 心跳写入 m_aecCalib"]
    peaks -->|"否: 峰数不足/疑似误锁"| retry{"继续累积至 6s 超时?<br/>超时注入 kAecBypass 解锁丢帧门(AEC 直通)<br/>(WARN 带 ncc/peaks/matched_slots)"}
    retry -->|"否"| pcm
...
- **版本化实例管理**：
  - 每个扩展 = `{name, version, wasm_bytes, wasm3 Runtime, 订阅表}`，按名称维护**双实例（current/next）**。
  - 新版本编译（`m3_ParseModule + m3_LoadModule`）成功后置为 next；在**心跳边界**原子切换（心跳内不切换，保证单轮语义一致），旧版本进入 draining，引用计数归零后卸载。
  - 编译/校验失败：拒绝切换，保留旧版本，上报 metric `wasm_reload_failures_total` 并告警。
- **资源隔离**（wasm3 无 fuel 概念）：每个 wasm 实例配置 runtime 线性内存上限（默认 64KB）+ 栈大小约束 + 单次调用在独立线程执行并计时超时（默认 5ms，超限/trap 计 `wasm_trap_total` 并熔断该实例 30s）。
...
| 会话 | 断线 3min 内重连恢复上下文（m_aecCalib 随连接失效，重连重新水印标定）；3min 后 GC 且 MemoryClient 被调用；重登踢旧连接后旧 gen 消息（含迟到断连）被丢弃 |
...
## 10A. 实现状态与选型偏差说明（2026-07 更新）

| 模块 | 状态 | 偏差说明 |
|------|------|----------|
| 四阶段心跳引擎 | ✅ 已落地 | 与本文档一致 |
| 三段式流水线+占位 | ✅ 已落地 | 含 A 未就绪复用上一次占位音 |
| WSS+JWT | ✅ 已落地 | HS256 自实现（HMAC-SHA256 常时间比较+exp），非 jwt-cpp；另有调试后门 token `debug:<uid>`（`--allow-debug-token` 开启，默认关闭，WARN 审计） |
| wasm 认证 | ✅ 已落地 | 运行时选型 **wasm3**（third_party 内置），fail-closed |
| wasm 观察者 | ✅ 已落地 v1 | 只读观察者（on_message ABI）+ trap 熔断（3 次/30s）；双向 host_emit_message 预留 |
| WebRTC APM | ✅ 已接入 | **偏差**：系统包 webrtc-audio-processing **0.3.1**，回声消除为 **AECM**（非 AEC3，AEC3 需 1.x 包）；NS(高抑制)/VAD 已启用；接口按 AEC3 用法设计（先 reverse 后 capture + 外部延迟补偿），升级仅改初始化 |
| 水印标定 | ✅ 已落地并 E2E 验证 | 客户端回环真实检出；间隔 320ms；边界 s=0 已修 |
| Redis | ✅ 已落地 | hiredis 同步 API（阶段4 线程池执行）；连接恢复 ctx/placeholder；断连降级纯内存态 |
| 指标 | ✅ 已落地 | **偏差**：未直接链 opentelemetry-cpp SDK（Ubuntu 22.04 无包，源码构建重）；实现 OTel 语义 API + Prometheus /metrics 端点，由 Collector prometheus receiver 转 OTLP；接口可无缝替换 SDK |
| 崩溃转储 | ✅ 已落地 | **偏差**：未用 Breakpad（无 CMake、autotools 集成重）；signal handler + backtrace_symbols_fd 写转储 + .meta，PIE 偏移符号化（scripts/symbolize.sh + addr2line），symbols target 抽 debuglink 符号；E2E 已验证 SIGSEGV 转储与符号化 |
| OTel trace | 未实现 | 后续接 SDK 时一并考虑 |

## 10B. convai.v1 协议接入（GoldieSettingsAndroid 联调，2026-07-22 落地）

**核心原则：核心代码零协议，协议全部在 wasm 插件内。**

- **路由**：`--protocol=<subproto>:<plugin.wasm>`（可多个逗号分隔）+ `--protocol-key=<配置槽内容>`。WsServer 握手按 `Sec-WebSocket-Protocol` 查路由表，命中进入插件会话；宿主只做裸帧搬运，不认识任何协议字段。
- **插件 ABI**（`ext/protocol_plugin.{h,cpp}` + `tests/wasm/convai_proto.wat`）：
  - 宿主派发：`on_ws_text / on_ws_binary / on_outbound_clip / on_outbound_text / on_llm_text / on_thinking`（全部原始载荷，13B 音频头也在插件内解析）
  - env 原语（协议无关）：`host_send_text/send_binary`（裸帧）、`host_inject`、`host_json_get`（顶层→body 回退）、`host_resample_g711`（泛型）、`host_bind_session/close/feed_watermark/uplink_asr/now_ms`
  - 插件自构完整信封（含 itoa 的 seq/ts）与 13B 音频头（BE）
- **鉴权**：设备 Key（调试约定 `goldie-dev-key-2026`）经配置槽 `[1024:len][1028:bytes]` 注入插件内存，hello.body.product_key 比对，不符 → hello_err + 关闭码 4401；`agent_id` 作 uid 派生 session_id。
- **采样率**：边界转换——下行 16k→8k（2:1 均值抽取）、上行 8k→16k（1:2 线性插值），引擎/APM/mock 全保持 16k。
- **水印标定**：每连接下行水印（16k chirp→8k 播放，端侧可闻"扫频脉冲组声纹"：4×20ms chirp、间隔 320ms、总时长 ~0.98s），上行回环在**协议侧 8k** 检测（模板 chirp 参数与源一致 1k~4k——降采样率不改变 Hz 频率；一致性投票：栅格命中 ≥count-2 且最深命中槽 ≥count-2，拒偏移子序列误锁），延迟 ×2 换算回 16k；6s 未命中注入 kAecBypass 解锁上行（AEC 直通）；标定随连接失效，重连重发重标。
- **测试音型**：mock TTS 按 clip 生成可听辨音型——安抚"叮-咚"、复述 do-mi-sol、答案"滴滴滴—长滴"，证明全链路通畅。
- **E2E**：`tools/convai_sim`（模拟 Android 客户端全消息流）+ `scripts/run_e2e_convai.sh`（含错误 Key 反例）。

### 联调中修复的关键缺陷

| 缺陷 | 根因 | 修复 |
|------|------|------|
| wasm 模块"malformed"随机失败 | **wasm3 不拷贝模块字节**，临时 vector 悬空（布局相关偶发） | Wasm3Module 持有 `m_bytes` 生命周期 |
| wasm 导入参数全错位 | `m3ApiReturnType` 必须先于 `m3ApiGetArg`（消费 _sp[0] 返回槽） | 全部 raw 函数调整顺序 |
| APM ProcessStream 恒 -11 | 0.3.1 的 stream delay 是**每帧消耗品**（用后复位） | 每帧处理前 set_stream_delay_ms |
| 安抚音不触发 | `OnAsrFinal` 清除 vadEndpoint，与 VAD 帧同心跳时条件被抹除 | 改为新语句开始时重置 |
| End 帧无 ASR final | convai End 空载荷被跳过，mock 收不到端点 flags | 空载荷带断句 flags 也写流 |
...
3. **wasm 选型：wasm3（已定）**：单文件 C 解释器，third_party 内置源码零外部依赖、启动微秒级，适合认证/观察者这类轻量扩展；代价是无 JIT（性能低于 Wasmtime）与无 fuel（靠内存上限+线程隔离超时熔断）。认证扩展已落地（§7.3），观察者消息扩展的 ABI 按同构设计预留。
