...
//
// 实现思路：
//   1. 会话开始录音前，下行注入"双声 chirp 水印"（1k→4kHz 扫频，两声、
//      固定间隔 GAP）。选 chirp 是因为宽带信号互相关峰尖锐，抗失真抗噪。
//   2. 在录回的 PCM 中与本地模板做滑动归一化互相关（NCC 匹配滤波），
//      非极大抑制提取峰值；找到间隔≈GAP 的双峰即标定成功：
